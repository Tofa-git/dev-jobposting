<div class="d-flex align-items-stretch flex-grow-1">
	<form action="{{ route('user-role.update', $data->id) }}" method="post" onsubmit="return globalFunction.checkSubmission(this, 0)" class="d-flex flex-column align-items-stretch w-100 h-100 m-0 p-0">
							...
							@foreach($list_menu as $_list_menu)
								<tr @if($_i % 2===1) class="bg-light" @else class="bg-white" @endif>
									...
									<td style="min-width: 200px; white-space: normal;">
										@if((int)$_list_menu->menu_type===4)
											<span class="fw-bold">{{ $_list_menu->caption }}</span>
										...
										@elseif((int)$_list_menu->menu_type===6)
											<span class="text-success">SEPARATOR</span>
										@endif
									</td>
									<td style="min-width: 200px; white-space: normal;">
										{{ $_list_menu->action }}
									...
									<td class="text-nowrap">
										@if($_list_menu->showMenu==='0')
											<div style="display: inline-block;" class="form-check">
												<input class="form-check-input" type="checkbox" @if($_list_menu->user_showMenu==='0') checked @endif id="showMenu{{ $_list_menu->id }}" name="showMenu{{ $_list_menu->id }}">
												<label class="form-check-label" for="showMenu{{ $_list_menu->id }}">Show Menu</label>
											</div>
										@endif
										@if($_list_menu->show==='0')
											<div style="display: inline-block;" class="form-check ms-2">
												<input class="form-check-input" type="checkbox" @if($_list_menu->user_show==='0') checked @endif id="show{{ $_list_menu->id }}" name="show{{ $_list_menu->id }}">
												<label class="form-check-label" for="show{{ $_list_menu->id }}">Index</label>
											</div>
										@endif
										@if($_list_menu->create==='0')
											<div style="display: inline-block;" class="form-check ms-2">
												<input class="form-check-input" type="checkbox" @if($_list_menu->user_create==='0') checked @endif id="create{{ $_list_menu->id }}" name="create{{ $_list_menu->id }}">
												<label class="form-check-label" for="create{{ $_list_menu->id }}">Create</label>
											</div>
										@endif
										@if($_list_menu->update==='0')
											<div style="display: inline-block;" class="form-check ms-2">
												<input class="form-check-input" type="checkbox" @if($_list_menu->user_update==='0') checked @endif id="update{{ $_list_menu->id }}" name="update{{ $_list_menu->id }}">
												<label class="form-check-label" for="update{{ $_list_menu->id }}">Update</label>
											</div>
										@endif
										@if($_list_menu->suspend==='0')
											<div style="display: inline-block;" class="form-check ms-2">
												<input class="form-check-input" type="checkbox" @if($_list_menu->user_suspend==='0') checked @endif id="suspend{{ $_list_menu->id }}" name="suspend{{ $_list_menu->id }}">
												<label class="form-check-label" for="suspend{{ $_list_menu->id }}">Suspend</label>
											</div>
										@endif
										@if($_list_menu->delete==='0')
											<div style="display: inline-block;" class="form-check ms-2">
												<input class="form-check-input" type="checkbox" @if($_list_menu->user_delete==='0') checked @endif id="delete{{ $_list_menu->id }}" name="delete{{ $_list_menu->id }}">
