...
<div class="d-flex flex-fill flex-column shadow-sm bg-white overflow-auto ms-2 mt-1" style="border-top-left-radius: 10px; border-bottom-left-radius: 10px; border-top: 1px solid #dddddd; border-left: 1px solid #dddddd; border-bottom: 1px solid #dddddd;">
	<form action="{{ route('posting-berita.update', $data->id) }}" method="post" enctype="multipart/form-data" class="d-flex flex-fill bg-white">
			...
			<div class="d-flex flex-fill flex-column">
					...
					<div class="col-sm-6">
						...
						<select class="form-select bg-white @error('jenis') is-invalid @enderror" id="jenis" name="jenis" required>
							@foreach($jenis_berita as $_jenis_berita)
								<option value="{{ $_jenis_berita->id }}" @if((int)$data->id_jenis === (int)$_jenis_berita->id) selected @endif>{{ $_jenis_berita->description }}</option>
							@endforeach
						</select>
					</div>
					<div class="col-sm-6">
						...
						<select class="form-select bg-white @error('kategori') is-invalid @enderror" id="kategori" name="kategori" required>
							@foreach($kategori_berita as $_kategori_berita)
								<option value="{{ $_kategori_berita->id }}" @if((int)$data->id_kategori === (int)$_kategori_berita->id) selected @endif>{{ $_kategori_berita->description }}</option>
							@endforeach
						</select>
					</div>
				</div>
				<div class="row mt-2 flex-shrink-1">
					<div class="col-sm-6">
						...
						<input type="text" class="form-control text-secondary bg-light @error('url_slug') is-invalid @enderror" name="url_slug" id="url_slug" value="{{ $data->slug }}" required />
					</div>
					<div class="col-sm-6">
						...
						@if(!is_null($data->foto_utama))
							<a href="#" target="_blank">Lihat foto utama</a>
						@endif
					</div>
				</div>
				<div class="row mt-2 flex-shrink-1">
					<div class="col-sm-6">
						...
						<div class="input-group">
							<input type="text" name="judul" id="judul" title="Judul Berita" placeholder="Judul Berita" class="auto_focus form-control bg-white @error('judul') is-invalid @enderror" required autofocus maxlength="255" value="{{ $data->title }}" onkeyup="globalFunction.updateJudul(this, '#url_slug')" />
					...
					<div class="col-sm-6">
						...
						<div class="input-group">
							<input type="text" name="keterangan_foto" id="keterangan_foto" title="Keterangan Foto Utama" placeholder="Keterangan Foto Utama" class="form-control bg-white @error('keterangan_foto') is-invalid @enderror" maxlength="255" value="{{ $data->keterangan_foto }}" />
							<div onclick="globalFunction.clearValue(event)" class="p-2 bg-secondary rounded-end-2 d-flex" style="cursor: pointer" title="Hapus">
								<i class="material-icons-outlined align-self-center text-white fs-5">clear</i>
							</div>
						</div>
					...
					<div class="tab-content flex-fill" id="nav-tabContent">
						...
						<div class="tab-pane fade show border-start border-end border-bottom p-2" id="nav-meta" role="tabpanel" aria-labelledby="nav-meta-tab" tabindex="0">
							<div class="mt-1">
								...
								<div class="d-flex">
									<input type="text" name="meta_title" id="meta_title" title="Meta Title" placeholder="Meta Title" class="w-100 form-control input-text @error('meta_title') is-invalid @enderror" value="{{ $data->meta_title }}" maxlength="255" />
									<div onclick="globalFunction.clearValue(event)" class="p-2 bg-secondary rounded-end-2 d-flex" style="cursor: pointer" title="Hapus">
										<i class="material-icons-outlined align-self-center text-white fs-5">clear</i>
									</div>
								</div>
