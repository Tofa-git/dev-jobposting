		...
		<div class="d-flex border-bottom">
			<div class="d-flex flex-grow-1 p-2">
				<form class="d-flex flex-grow-1">
					<input type="hidden" name="jenis" value="{{ @$_REQUEST['jenis'] }}" />
					...
					<div class="flex-grow-1 input-group ms-2 d-none d-sm-flex">
						<input name="q" type="text" class="form-control bg-white border-grey" value="{{ @$_REQUEST['q'] }}" placeholder="Judul Berita">
			...
			<div class="p-2 d-flex">
				<a href="{{ route('posting-berita.create') }}" class="p-1 px-2 d-flex align-items-center btn btn-warning bg-gradient p-0" role="button">
		...
		<table class="table table-striped table-hover w-100" cellpadding="0" cellspacing="0">
			<thead>
				<tr class="text-nowrap">
					<th width="30px">No</th>
					<th width="100px">Jenis dan Kategori</th>
						...
						<tr @if($_i % 2===0) class="bg-light" @else class="bg-white" @endif valign="top">
							...
							<td>
								<div  class="d-flex flex-column">
									<span>{{ $_data->jenisBerita->description }}</span>
									<span>{{ $_data->kategoriBerita->description }}</span>
								</div>
							</td>
							<td>
								<div class="d-flex">
									<div style="max-width: 150px; min-width: 150px; height: 100px; background-size: cover; background-position: center; background-repeat: no-repeat; background-image: url({{ \App\Models\data_file::getThumbnailImage($_data->foto_utama) }});" class="border"></div>
									<div class="px-2">
										<div class="fw-bold">{{ $_data->title }}</div> 
										<div>Target URL : <span class="text-primary">{{ $_data->slug }}</span></div>
										<div>Dibuat oleh : <span class="text-primary">{{ @$_data->postUser->name.'<'.@$_data->postUser->email.'>' }}</span></div>
									</div>
								</div>
							</td>
							<td>{{ $_data->published_at }}</td>
							<td>
								@if(\Auth::user()->hasPermission('Posting Berita', 'suspend'))
									@if((int)$_data->published_by > 0)
										<a href="{{ route('posting-berita.publish', $_data->id) }}" role="button" class="btn btn-sm btn-primary bg-gradient m-0 p-0 ps-2 pe-2" title="Click for suspend">PUBLISH</a>
									@else
										<a href="{{ route('posting-berita.publish', $_data->id) }}" role="button" class="btn btn-sm btn-secondary m-0 p-0 ps-2 pe-2" title="Click for activate">DRAFT</a>
									@endif
								@endif
							</td>
							<td>
								@if(\Auth::user()->hasPermission('Posting Berita', 'suspend'))
									@if($_data->status==='0')
										<a href="{{ route('posting-berita.status', $_data->id) }}" role="button" class="btn btn-sm btn-info bg-gradient m-0 p-0 ps-2 pe-2" title="Click for suspend">ACTIVE</a>
									@elseif($_data->status==='1')
										<a href="{{ route('posting-berita.status', $_data->id) }}" role="button" class="btn btn-sm btn-warning m-0 p-0 ps-2 pe-2" title="Click for activate">SUSPEND</a>
									@elseif($_data->status==='3')
										<a href="{{ route('posting-berita.status', $_data->id) }}" role="button" class="btn btn-sm btn-secondary m-0 p-0 ps-2 pe-2" title="Click for activate">DRAFT</a>
									@endif
								@endif
							</td>
							<td class="text-nowrap">
								@if(is_null($_data->deleted_at))
									@if(\Auth::user()->hasPermission('Posting Berita', 'update'))
										<a href="{{ route('posting-berita.edit', $_data) }}" class="btn btn-outline-primary btn-sm p-0 px-2" title="Edit" role="button"><i class="material-icons-outlined p-1 d-flex fs-6">create</i></a>
									@endif
									@if(\Auth::user()->hasPermission('Posting Berita', 'delete'))
										<form method="post" onsubmit="return confirm('Are you sure want to delete this record?')" action="{{ route('posting-berita.destroy', $_data) }}" class="d-inline">
											@csrf()
											@method('delete')
									...
									@endif
								@else
									<a onclick="return confirm('Are you sure want to restore this record?')" href="{{ route('posting-berita.restore', $_data) }}" class="btn btn-outline-outline-danger btn-sm p-0 px-2" title="Restore" role="button"><i class="material-icons-outlined p-1 d-flex fs-6">restore_page</i></a>
								@endif
							</td>
						</tr>
						@php $_i++; @endphp
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
	...
	<div class="offcanvas-body">
		<form>
			...
			<div class="row mt-1">
				<div class="col-sm-4 small">Jenis</div>
				<div class="col-sm-8">
					<select class="form-select bg-white" name="jenis">
						<option value="0" selected>Semua Jenis Berita</option>
						@foreach($jenis_berita as $_jenis_berita)
							<option value="{{ $_jenis_berita->id }}" @if((int)@$_REQUEST['jenis']===(int)$_jenis_berita->id) selected @endif>{{ $_jenis_berita->description }}</option>
						@endforeach
					</select>
				</div>
			</div>
			<div class="row mt-1">
				<div class="col-sm-4 small">Kategori</div>
				<div class="col-sm-8">
					<select class="form-select bg-white" name="kategori">
						<option value="0" selected>Semua Kategori Berita</option>
						@foreach($kategori_berita as $_kategori_berita)
							<option value="{{ $_kategori_berita->id }}" @if((int)@$_REQUEST['kategori']===(int)$_kategori_berita->id) selected @endif>{{ $_kategori_berita->description }}</option>
						@endforeach
					</select>
				</div>
			</div>
			<div class="row mt-1">
				...
				<div class="col-sm-8"><input type="text" class="form-control bg-white" name="q" placeholder="Judul Berita" value="{{ @$_REQUEST['q'] }}" /></div>
			</div>
			<hr />
