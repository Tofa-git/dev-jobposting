...
class logActivities
{
    ...
    public static function addToLog($module, $title, $description, $type)
    {
        $agent = logActivities::getBrowserInfo();
        $log = [];
        $log['ip4']             = Request::ip();
        $log['module']          = $module;
        $log['title']           = $title;
        ...
        $log['platform']        = $agent['os_platform'];
        $log['agent']           = $agent['user_agent'];
        $log['browser']         = $agent['browser'];
        $log['browser_version'] = $agent['browser_version'];
        $log['log_type']        = $type;
        $log['status']          = '0';
        $log['created_by']      = \Auth::check() ? \Auth::user()->id : 0;
        log_activity::create($log);
    }
